...
def output(infn, docnum, sentences):
    ...
    for si, sentence in enumerate(sentences):
        ...
        for ID, form, POS, prob in tokens:
...
def process(fn):
    ...
    with codecs.open(fn, encoding=INPUT_ENCODING) as f:
        ...
        for ln, l in enumerate(lines):
            ...
            assert len(fields) == 7, "Format error on line %d in %s: %s" % (ln, fn, l)

            ID, form, POS, prob = fields[0], fields[1], fields[3], fields[6]
            head, rel = fields[5], fields[5]

            tokens.append((ID, form, POS, prob))
